...
import { TripleVoiceInputLiveKit, TranscriptionData } from './TripleVoiceInputLiveKit';
import { TranscriptionResults, TranscriptionData as ResultData } from './TranscriptionResultsTriple';
...
export const VoiceExerciseWithLiveKit: React.FC = () => {
  ...
  const handleServicesReady = (webSpeechReady: boolean, deepgramReady: boolean, groqReady: boolean) => {
    // For LiveKit, we consider services ready when at least one service through LiveKit is ready
    const anyServiceReady = webSpeechReady || (deepgramReady && groqReady);
    setServicesReady(anyServiceReady);
    
    // Start timer only when services are ready and we haven't started yet
    if (anyServiceReady && !timerStarted && !showFeedback) {
      setProblemStartTime(Date.now());
      setTimerStarted(true);
      console.log('⏱️ Services ready - starting timer');
    }
  };
      ...
      <div className="max-w-4xl mx-auto">
        ...
        <div className="mb-8">
          <div className="flex justify-between mb-2">
            ...
            <span className="text-sm text-gray-600">
              Triple Voice Input Test (Web Speech + Deepgram + Groq via LiveKit)
            </span>
          </div>
        ...
        <div className="mt-8 text-center text-gray-600">
          ...
          <p className="text-sm mt-2">Web Speech API, Deepgram, and Groq/Whisper are listening via LiveKit</p>
        </div>
      </div>
    </div>
  );
