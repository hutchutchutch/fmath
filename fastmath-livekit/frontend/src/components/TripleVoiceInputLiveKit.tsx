...
import { Room, RoomEvent, LocalTrack, LocalAudioTrack, Track, createLocalTracks } from 'livekit-client'
...
interface TripleVoiceInputLiveKitProps {
  question: {
    num1: number;
    num2: number;
    operator: string;
...
export const TripleVoiceInputLiveKit: React.FC<TripleVoiceInputLiveKitProps> = ({ 
  question, 
  onAnswer, 
  showFeedback, 
  enableVoice = true,
  ...
  const connectToLiveKit = async () => {
    try {
      ...
      // Join backend services to the room
      ...
      console.log('🎯 Backend services joined room');
      ...
      if (tracks.length > 0 && tracks[0].kind === Track.Kind.Audio) {
        ...
        if (groqEnabled) {
          setIsGroqReady(true);
          setGroqStatus('Connected via LiveKit');
        }
      }
      ...
      setGroqStatus('LiveKit connection failed');
    }
  };
  ...
  const handleDeepgramToggle = () => {
    ...
    if (newState && isListening && !roomRef.current) {
      connectToLiveKit();
    } else if (!newState) {
      setIsDeepgramReady(false);
