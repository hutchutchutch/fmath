...
interface AccuracyTextInputProps {
  question: {
    num1: number;
    num2: number;
    operator: string;
...
export const AccuracyTextInput: React.FC<AccuracyTextInputProps> = ({ 
  question, 
  onAnswer, 
  showFeedback,
  isCorrect 
  ...
  const [lastQuestionId, setLastQuestionId] = useState<string>(questionId); // Initialize with current questionId

  // Reset input when question actually changes - not on timer zone changes
  useEffect(() => {
    ...
    if (isNewQuestion && !showFeedback) {
      setValue('');
      setUserAnswer('');
      setLastAnswer(null); // Keep this if it's desired behavior for Accuracy practice
      setLastQuestionId(questionId);
    }
  }, [questionId, lastQuestionId, showFeedback]);

  // Handle feedback state transitions (e.g., when feedback is hidden after being shown)
  useEffect(() => {
    ...
    if (previousFeedbackRef.current && !showFeedback) {
      setValue('');
      setUserAnswer('');
      // Do not setLastAnswer(null) here, as the question might not have changed yet.
      // The effect above handles new questions.
    }
...
export default AccuracyTextInput; 