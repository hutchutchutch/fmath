...
const QuestionScreen: React.FC<QuestionScreenProps> = ({ onEnd, settings, questions: initialQuestions }) => {
  ...
  const handlePracticeEnd = async () => {
    try {
      ...
      // Process all typing data first
      if (typingDataQueue.current.length > 0) {
        try {
      ...
      const allAttemptedQuestions = initialQuestions.slice(0, questionsAnswered);

      // Complete the assessment with additional timing data
      // Calculate actual test duration in minutes
      const testDurationMs = Date.now() - testStartTime.current;
      const testDurationMinutes = testDurationMs / 60000; // Convert to minutes
      ...
      while (attempts < MAX_ATTEMPTS && !assessmentResult) {
        try {
          assessmentResult = await completeProgressAssessment(settings.assessmentId, user.userId, {
            ...
            actualDurationMinutes: testDurationMinutes, // Pass actual duration
            clientSideStats: {
  ...
  const handleAnswer = async (answer: number, typingData?: { count: number; time: number }) => {
    ...
    if (user?.userId) {
      try {
        ...
        // 2. Update user progress using queue
