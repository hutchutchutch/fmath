...
import { FluencyPracticeSettings } from './FluencyPracticePage';

interface SelectionScreenProps {
  onStart: (settings: FluencyPracticeSettings) => void;
  facts: Fact[];
...
const SelectionScreen: React.FC<SelectionScreenProps> = ({ 
  ...
  const handleCountdownComplete = () => {
    try {
      onStart({
        facts,
        timerDuration: 6 // This value will be overridden by the parent component
      });
    } catch (error) {
      console.error('Failed to start Fluency practice:', error);
      setIsLoading(false);
              ...
              <div className="text-center space-y-6">
                ...
                <ul className="space-y-3 text-base text-gray-600 max-w-2xl mx-auto text-left">
                  <li className="flex items-center gap-4">
                    ...
                    <span>You'll practice with progressively faster timer speeds in a single session</span>
                  </li>
                  <li className="flex items-center gap-4">
                    <span className="font-bold text-blue-500 min-w-[1.5rem]">2.</span>
                    <span>Starting with 6 seconds, then 3s, 2s, 1.5s, and finally 1s (if available)</span>
                  </li>
                  <li className="flex items-center gap-4">
                    <span className="font-bold text-blue-500 min-w-[1.5rem]">3.</span>
                    <span>Answer each question as fast as you can</span>
                  </li>
                  <li className="flex items-center gap-4">
                    <span className="font-bold text-blue-500 min-w-[1.5rem]">4.</span>
                    <span>If you don't answer within the time limit, the answer will be shown</span>
                  </li>
                  <li className="flex items-center gap-4">
