...
echo -e "${PURPLE}╔════════════════════════════════════════╗${NC}"
echo -e "${PURPLE}║     FastMath Deepgram Test System      ║${NC}"
echo -e "${PURPLE}╚════════════════════════════════════════╝${NC}"
echo ""
...
EOF
    ...
    echo "  2. Get LiveKit API key and secret from: https://cloud.livekit.io/"
    echo ""
...
if grep -q "DEEPGRAM_API_KEY=your_deepgram_key_here" .env || ! grep -q "DEEPGRAM_API_KEY=.." .env; then
...
if grep -q "LIVEKIT_API_KEY=your_livekit_key_here" .env || ! grep -q "LIVEKIT_API_KEY=.." .env; then
    echo -e "  ${RED}✗ LIVEKIT_API_KEY not configured${NC}"
    MISSING_KEYS=true
else
    echo -e "  ${GREEN}✓ LIVEKIT_API_KEY configured${NC}"
fi

if grep -q "LIVEKIT_API_SECRET=your_livekit_secret_here" .env || ! grep -q "LIVEKIT_API_SECRET=.." .env; then
    echo -e "  ${RED}✗ LIVEKIT_API_SECRET not configured${NC}"
    MISSING_KEYS=true
else
    echo -e "  ${GREEN}✓ LIVEKIT_API_SECRET configured${NC}"
fi

if grep -q "LIVEKIT_URL=wss://your-project.livekit.cloud" .env || ! grep -q "LIVEKIT_URL=.." .env; then
    echo -e "  ${RED}✗ LIVEKIT_URL not configured${NC}"
    MISSING_KEYS=true
else
...
if [ "$MISSING_KEYS" = true ]; then
    echo ""
    echo -e "${RED}Please configure the missing API keys in .env${NC}"
    exit 1
fi
...
echo -e "   2. Click '${GREEN}Start Test${NC}' button"
echo -e "   3. Allow microphone access"
echo -e "   4. Speak numbers clearly: ${YELLOW}'three'${NC}, ${YELLOW}'seven'${NC}, ${YELLOW}'nine'${NC}"
echo ""
...
echo -e "   ✅ Connected to room"
echo -e "   📡 Track subscribed"
echo -e "   🎵 Audio track detected"
